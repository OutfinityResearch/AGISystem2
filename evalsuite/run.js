        ...
        switch (command) {
          case 'ASK':
            if (canonical.subject && canonical.relation && canonical.object) {
              // v3 syntax: @var Subject VERB Object (no ASK verb)
              return `@q ${canonical.subject} ${canonical.relation} ${canonical.object}`;
            }
            break;
...
function discoverCases(options = {}) {
  ...
  for (const entry of entries) {
    ...
    // Support both case.json and case.js formats
    ...
    if (caseData) {
      // Normalize legacy case formats (case.js with theory_NL/theory_DSL/tasks)
      // into the unified { theory, queries } shape expected by evaluators.
      if (!caseData.theory && (caseData.theory_NL || caseData.theory_DSL)) {
        caseData.theory = {
          natural_language: caseData.theory_NL || '',
          expected_facts: Array.isArray(caseData.theory_DSL) ? caseData.theory_DSL : []
        };
      }

      if (!caseData.queries && Array.isArray(caseData.tasks)) {
        caseData.queries = caseData.tasks.map((task) => {
          const query = {
            id: task.id,
            natural_language: task.TASK_NL || '',
            expected_dsl: task.TASK_DSL || ''
          };
          // Map legacy answer fields into expected_answer when available
          if (task.ANSWEAR_NL || task.ANSWEAR_DSL || task.PROOF_DSL) {
            let truth = null;
            try {
              if (task.ANSWEAR_DSL) {
                const parsed = JSON.parse(task.ANSWEAR_DSL);
                truth = parsed.truth || null;
              }
            } catch {
              // Ignore parse errors; leave truth as null
            }
            query.expected_answer = {
              natural_language: task.ANSWEAR_NL || '',
              truth: truth,
              dsl_explanation: task.PROOF_DSL || ''
            };
          }
          return query;
        });
 ...
 * Supports both legacy format and v3.0 format
 */
function normalizeTestCase(testCase) {
  // Check if this is v3.0 format (has theory_DSL and tasks)
  if (testCase.theory_DSL && testCase.tasks) {
    // Convert v3.0 format to legacy format
    return {
      id: testCase.id,
      name: testCase.name,
      _dir: testCase._dir,
      ...
      queries: (testCase.tasks || []).map(task => {
        // Parse ANSWEAR_DSL as JSON if it's a string
        let expectedAnswer = {};
        if (task.ANSWEAR_DSL) {
          try {
            expectedAnswer = typeof task.ANSWEAR_DSL === 'string'
              ? JSON.parse(task.ANSWEAR_DSL)
              : task.ANSWEAR_DSL;
          } catch (e) {
            expectedAnswer = { truth: 'TRUE_CERTAIN' };
          }
        }
        return {
          id: task.id,
          ...
          expected_dsl: task.TASK_DSL || '',
          expected_answer: {
            truth: expectedAnswer.truth || 'TRUE_CERTAIN',
            method: expectedAnswer.method,
            chain: expectedAnswer.chain,
            natural_language: task.ANSWEAR_NL || ''
          },
...
async function evaluateCase(testCase, executor, options) {
  ...
  if (isDirectDsl && testCase.theory.expected_facts) {
    // v3 syntax: @varName Subject VERB Object - unique names for topological order control
    const dslLines = testCase.theory.expected_facts.map((fact, idx) =>
      `@f${idx} ${fact}`
    );
      ...
      if (constraints.disjoint && Array.isArray(constraints.disjoint)) {
        ...
        for (const pair of constraints.disjoint) {
          if (Array.isArray(pair) && pair.length >= 2) {
            // v3 syntax: @varName Subject DISJOINT_WITH Object - unique names for tracking
            dslLines.push(`@disj${disjIdx} ${pair[0]} DISJOINT_WITH ${pair[1]}`);
            disjIdx++;
          }
        }
