...
import { polynomialTermCount, vectorValue } from '../lib/views.mjs';
...
export async function handleScopeApi(req, res, url, ctx) {
  ...
  const filtered = (q ? names.filter(n => String(n).toLowerCase().includes(q)) : names)
    .filter((n) => {
      ...
      const vec = session?.scope?.get?.(n) || null;
  ...
  const bindings = page.map((name) => {
    ...
    const src = session?.referenceMetadata?.get?.(String(name))?.source || null;
    return { name: String(name), vectorValue: vectorValue(vec), source: src };
  });
