...
describe('DS19: Semantic Unification — proof schema + validator', () => {
  test.skip(
    ...
    () => {
      ...
      assert.equal(typeof result.proof, 'object');
      assert.equal(typeof result.proof.valid, 'boolean');
      assert.deepEqual(result.proof.goal, { operator: 'isA', args: ['Socrates', 'Human'] });
  ...
  test.skip(
    ...
    async () => {
      ...
      const ok = validateProof(result.proof, session);
