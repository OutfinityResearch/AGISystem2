...
- **FS-09 Interaction Layer:** Expose a deterministic interaction layer built around `AgentSystem2` and `System2Session`. External callers obtain a `System2Session` and send it Sys2DSL command lines (possibly derived from natural language via a translator). The core engine receives only structured, session-scoped commands; all fact creation, updates, and queries, including theory imports/exports, are mediated by this layer.
