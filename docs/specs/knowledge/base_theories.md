...
## Notes
Dimension indices mentioned here follow the ontology and axiology catalogs; implementations must use those catalogs rather than ad-hoc numbering.

- The **canonical representation** of each theory is a single Sys2DSL text file (for example, `theories/base/root.sys2dsl`, `theories/law/minimal.sys2dsl`, `theories/law/conflictA.sys2dsl`), suitable for storage in git and manual review.
- Binary forms, if used, are purely caches that can be regenerated from the Sys2DSL text and must never be treated as the source of truth.
- Child layers never mutate parent files; overrides are expressed in the Sys2DSL programmes and applied by `TheoryStack` at runtime in a deterministic order.***
