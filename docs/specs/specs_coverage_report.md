...
- FastEval implements DS14’s “one suite = one Session” model in `evals/fastEval/lib/runner.mjs`, but timeouts and NL-to-DSL validation differ slightly from the spec (uses soft timeouts and allows transformer output drift).
...
| DS14 EvalSuite | `evals/fastEval/lib/runner.mjs` enforces one Session per suite and dual-input flows | Implemented, though spec’s strict NL-vs-DSL comparison is relaxed in code. |
...
## Config mapping
- Each `config/Core/*.sys2` file now has a spec under `docs/specs/config/`. Loading coverage remains in `tests/unit/runtime/core-theories.test.mjs` and FastEval’s loader (`evals/fastEval/lib/runner.mjs`).
