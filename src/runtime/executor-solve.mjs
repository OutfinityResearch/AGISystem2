...
function sourceFromStmt(stmt, comment) {
  const src = stmt?.source || null;
  return {
    file: src?.file ?? null,
    line: Number.isFinite(src?.line) ? src.line : (Number.isFinite(stmt?.line) ? stmt.line : null),
    column: Number.isFinite(src?.column) ? src.column : (Number.isFinite(stmt?.column) ? stmt.column : null),
...
export function executeSolveBlock(executor, stmt) {
  ...
  if (result.success && solutionVectors.length > 0) {
    const cspTupleOp = executor.session.vocabulary.getOrCreate('cspTuple', {
      source: sourceFromStmt(stmt, null),
      comment: 'Created/ensured for solve(csp) tuple facts.'
    });
    for (let i = 0; i < solutionVectors.length; i++) {
      ...
      executor.session.kbFacts.push({
        ...
        metadata: {
          ...
          proof: proofText,
          source: sourceFromStmt(stmt, 'Generated by solve(csp) compound solution record.')
        }
      });
      ...
      for (const { entity, value } of assignments) {
        ...
        executor.session.addToKB(factVec, `${solutionRelation}_${entity}_${value}`, {
          ...
          args: [entity, value],
          source: sourceFromStmt(stmt, 'Generated by solve(csp) solution binding.'),
          solutionIndex: i + 1,
          proof: proofText
        });
      }
      ...
      executor.session.addToKB(tupleVec, `${solutionName}_tuple`, {
        operator: 'cspTuple',
        args: tupleArgs,
        source: sourceFromStmt(stmt, 'Generated by solve(csp) deterministic tuple encoding.'),
        proof: proofText,
        solutionRelation,
        solutionIndex: i + 1
      });
...
function executePlanningSolveBlock(executor, stmt) {
  ...
  session.addToKB(planVec, `${planName}_plan`, {
    ...
    source: sourceFromStmt(stmt, 'Generated by solve(planning) plan summary.'),
  ...
  for (let i = 0; i < planning.plan.length; i++) {
    ...
    session.addToKB(vec, `${planName}_step${stepIndex}`, {
      ...
      source: sourceFromStmt(stmt, 'Generated by solve(planning) step fact.')
    });
    ...
    if (sig) {
      ...
      session.addToKB(actionVec, `${planName}_action${stepIndex}`, {
        operator: 'planAction',
        args: actionArgs,
        source: sourceFromStmt(stmt, 'Generated by solve(planning) action fact.')
      });
